...
PC1 = [57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,
        11,3,60,52,44,36,63,55,47,39,31,23,15,7,62,54,46,38,30,22,
...
# ==== Enkripsi & Dekripsi ====
def DES(block, keys, enc=True):
    block = permute(block, IP)
    L, R = block[:32], block[32:]
    order = keys if enc else keys[::-1]
    for k in order:
        L, R = R, xor(L, F(R, k))
...
# ==== Mode ECB ====
def encrypt_ecb(plaintext, keys):
    blocks = [plaintext[i:i+8] for i in range(0, len(plaintext), 8)]
    ciphertext = ""
    for block in blocks:
        pt_bits = text_to_bits(block.ljust(8, ' '))  # padding spasi
        ct_bits = DES(pt_bits, keys, True)
        ciphertext += bits_to_hexstr(ct_bits)
    return ciphertext

def decrypt_ecb(ciphertext_hex, keys):
    plaintext = ""
    for i in range(0, len(ciphertext_hex), 16):  # 16 hex = 64 bit
        block_hex = ciphertext_hex[i:i+16]
        bits = [(int(block_hex,16)>>j)&1 for j in range(63,-1,-1)]
        pt_bits = DES(bits, keys, False)
